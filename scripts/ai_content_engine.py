...
# 2026年TikTok最適化: 3-5個、ニッチ+中規模+大規模のミックス
# #ナースロビー は「紹介」カテゴリ以外では使わない（宣伝感回避）
# #AI は飽和タグなので #AIにやらせてみた #AI活用 等の中規模タグに置換
HASHTAG_SETS = {
    "あるある": [
        ["#看護師あるある", "#夜勤あるある", "#ナースの本音", "#AIにやらせてみた"],
        ["#看護師あるある", "#看護師の日常", "#病棟あるある", "#ナース"],
        ["#看護師あるある", "#申し送り", "#夜勤明け", "#ナースあるある"],
        ["#看護師あるある", "#ナースコール", "#看護師の日常", "#AIやってみた"],
        ["#看護師あるある", "#先輩看護師", "#新人ナース", "#看護師の日常"],
    ],
    "転職": [
        ["#看護師転職", "#転職活動中", "#神奈川看護師", "#年収アップ"],
        ["#看護師転職", "#キャリアチェンジ", "#看護師の働き方", "#転職相談"],
        ["#看護師転職", "#経験年数", "#市場価値", "#キャリア"],
        ["#看護師転職", "#退職", "#転職理由", "#看護師の本音"],
    ],
    "給与": [
        ["#看護師", "#年収", "#給与明細", "#手当比較"],
        ["#看護師転職", "#夜勤手当", "#給料日", "#年収アップ"],
        ["#看護師", "#給料比較", "#神奈川", "#手取り公開"],
        ["#看護師", "#年収格差", "#地域差", "#知らないと損"],
    ],
    "紹介": [
        ["#看護師転職", "#ナースロビー", "#手数料10パーセント", "#転職エージェント"],
        ["#神奈川看護師", "#ナースロビー", "#転職サポート"],
        ["#看護師転職", "#ナースロビー", "#手数料比較", "#神奈川"],
    ],
    "トレンド": [
        ["#看護師あるある", "#看護師の日常", "#ナースの本音", "#共感"],
        ["#看護師", "#ナース", "#病院あるある", "#共感したらいいね"],
        ["#看護師あるある", "#看護師の彼女", "#夜勤の現実", "#ナース"],
        ["#看護師", "#医療従事者", "#ナースの日常", "#共感"],
    ],
}
...
CONTENT_STOCK_HINTS = {
    ...
    "紹介": [
        "手数料10% vs 30%→差額で看護師1人分の年間研修費が出る。病院が喜ぶ理由",
        "転職エージェント選びで一番大事なこと→AIの回答「手数料率を聞け」",
    ],
    "トレンド": [
...
## フック公式（7パターンをローテーション）
1. 質問型: 「夜勤明け3連続ナースコール、耐えた人いる？」
2. 対立型: [他者] + [否定] → AIで見せた → [相手が黙った/泣いた]
3. 数字型: 「看護師5年目、年収100万の差がつく理由」
4. 告白型: 「正直に言う。転職が怖いのは○○のせい」
5. 衝撃型: 「これ知らない看護師、損してる」
6. 共感型: 「わかる人だけわかって。夜勤明けの○○」
7. 比較型: 「○○ vs ○○、どっちが正解？」

## コンテンツ公式
[看護師の超具体的な場面] + [AIにやらせてみた] → [予想外の結果・感情が動く展開]
※「夜勤あるある」ではなく「夜勤明け3連続ナースコールあるある」レベルの具体性

## スライド構成（7枚 — スワイプ率60%以上を目指す）
1枚目: フック（20文字以内。パターンインタラプト=予想を裏切る一言。3秒で止める）
2枚目: 共感（「あるある」で「自分のことだ」と思わせる。具体的シーン描写）
3枚目: 展開（AIに聞いた/やらせた。好奇心を維持）
4枚目: 深掘り（データや事実。「知らなかった」を生む）
5枚目: クライマックス（衝撃の結果。感情のピーク）
6枚目: オチ+CTA（余韻を残す。次のアクションへ自然に誘導）
7枚目: ブランドスライド（自動生成されるので台本不要）

## キャプション構造（2026年TikTok最適化）
- 1行目: 感情フック（質問形式が最も効果的。改行で分離）
- 2-3行目: ストーリーの核心（共感ポイント。改行で区切る）
- 最終行: CTA（自然に。押し売り厳禁）
...
- 患者の個人情報に触れない
- 実在施設の批判をしない
- ハッシュタグ3-4個（5個以内厳守。スパム判定回避）

## 品質基準
- 1枚目フックは20文字以内。パターンインタラプト必須
- キャプション1行目は質問形式が理想
...
- slides: 6枚分のテキスト（配列）
  - slides[0]: 1枚目フック（hookと同じでOK。パターンインタラプト=予想を裏切る一言）
  - slides[1]: 共感スライド（「あるある」で「自分のことだ」と思わせる。超具体的なシーン描写）
  - slides[2]: 展開（AIに聞いた/やらせた。好奇心を維持。「え、マジ？」と思わせる）
  - slides[3]: 深掘り（データや事実。「知らなかった…」を生む。保存したくなる情報）
  - slides[4]: クライマックス（衝撃の結果。感情のピーク。一番シェアしたくなる瞬間）
  - slides[5]: 6枚目 オチ+CTA（余韻+次のアクション。ロビーのまとめ）
- caption: SNSキャプション。以下の構造を厳守:
  1行目: 感情フック（質問形式推奨。例:「夜勤明け3連続ナースコール、耐えた人いる？」）
  （改行）
  2-3行目: 核心の共感ポイント（短文で区切る）
  （改行）
  最終行: CTA（自然に。保存/フォロー/コメント誘導）
  ※改行は \\n で表現。合計200文字以内。
- hashtags: ハッシュタグ（3-4個。ニッチタグ+中規模タグ+大規模タグのミックス）
...
## 超具体性の例（この粒度で書け）
BAD: 「夜勤あるある」
GOOD: 「夜勤明け3連続ナースコールで仮眠0分あるある」
BAD: 「先輩が怖い」
GOOD: 「申し送りで『それ昨日も言ったよね？』って言われた時の背筋」
BAD: 「給料が安い」
GOOD: 「手取り24万。同期のIT企業の友達は32万。飲み会で給料の話になった時の顔」
